...
- investigation 是第一阶段。不要读取 STATE 或 scientist-owned runs；该部分尚未发生或属于后续 experiment-reviewer。
...
1. Claim coverage: INVES.md I0 是否拆清楚 explicit claims、implicit generality claims、artifact/protocol/data dependencies 和 stated scope？目标论文显式 claim 是否使用稳定 `C<number>`，investigation-only 项是否使用 `IC<number>`？
...
6. Literature coverage: cited-by、references、作者后续工作、直接复现/失败复现、社区争议是否足够支撑当前 investigation findings?
   `lit-feed.md` 中 intended_reader 包含 investigator 的条目必须已由 investigator 消费，否则不得 ready。
...
禁止读取或写入 STATE.md；禁止写 INVES.md I0-I6、coder outputs、auditor reports 和最终报告。
